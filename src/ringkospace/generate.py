...
Two modes (see docs/SUMMARY-20260906.md, exposure-bias section):
  ...
  stream  prime states on the prompt, then step one byte at a time carrying
          the state. True O(1)/token inference, but v0 was not trained for
          single-step roll-out -> exposure bias (degeneration). The fix is the
          streaming-state training objective (next generation), not an
          architecture bug.
...
def generate(model: M.RingKoSSM, prompt: list[int], n: int, temperature: float,
             ...
             mode: str = "stream", device: str = "cuda") -> tuple[list[int], str]:
    g = torch.Generator(device=device).manual_seed(seed)
    seq = prompt[: maxlen - 1]
    done: list[int] = []
    enc = lambda toks: torch.tensor(toks, dtype=torch.long, device=device).unsqueeze(0)
    with torch.autocast(device, dtype=torch.bfloat16):
        lg, states = model(enc(seq))  # prime states on prompt
        logits = lg[0, -1].float()
        for _ in range(n):
            if mode == "window":
            ...
            if mode == "stream":
                lg, states = model(enc([nxt]), states)
                logits = lg[0, -1].float()
            else:
                seq = (seq + [nxt])[-maxlen:]
    return done, decode([t for t in done if BYTE_OFFSET <= t < 260])
...
def main() -> None:
    ...
    model = M.RingKoSSM(cfg["dim"], cfg["layers"], evolve=bool(cfg.get("evolve", 0))).to("cuda").eval()
    ...
    gen, text = generate(model, pr, args.tokens, args.temp, args.topk, args.seed,
                         args.maxlen, rep_pen=args.rep_pen, mode=args.mode)
